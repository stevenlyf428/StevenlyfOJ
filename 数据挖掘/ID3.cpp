...
// } _attributeTypeName;
//决策树 ID3 算法代码实现
//Copyright:wxh5055 #include <stdio.h> #include <stdlib.h> #include <math.h> #include <string.h>
...
typedef struct _examples
{
    ...
    _attributeTypeName *pAttributeTypeName; //训练样例集的属性类型int *pTrainSample; //训练样例集
} _examples, *pExamples;

//决策树结点结构体
typedef struct _node
{
    _attributeTypeName attributeTypeName; //节点代表的属性类型名称int childNum;//分支数
    int *pChildAttributeValue;            //每个子节点对应的属性值struct _node **pChildNode;//指向子结点
...
//定义每个属性类型和属性值之间的连接符char *connectString="--->";
...
void CalcPosAndNegNum(pExamples pNodeExamples, int *pPosNum, int *pNegNum)
{
    ...
    *pNegNum = 0;
    ...
    {
        //pTrainSample	为   训   练   样   例   首   地  址   ，   每  个   训   练   样  本   共   有
        (pNodeExamples->attributeTypeNum + 1(目标属性)) 个属性
            //根据目标属性地址存储的 yes 或 no 值来增加 pPosNum 或 pNegNum

            if (*(pTrainSample + i * ((pNodeExamples->attributeTypeNum) + 1) + pNodeExamples->attributeT ypeNum) == yes)
        {
            (*pPosNum)++;
        }

        if (*(pTrainSample + i * ((pNodeExamples->attributeTypeNum) + 1) + pNodeExamples->attributeT ypeNum) == no)
        {
            (*pNegNum)++;
        }
    }
...
double CalcEntropy(int posNum, int negNum)
{
    ...
    double entropy;
    //如果输入正负样本数有一个为 0,则返回熵值为 0 if(posNum==0 || negNum==0)
    {
    }
    else
    {
    ...
    //把 int 型转化为 double 型double fPosNum=(double)posNum; double fNegNum=(double)negNum;
    //计算正负样本数的比例
...
int FindMaxInfoGainAttrbute(pExamples pNodeExamples, int posNum, int negNum, int *pMaxInfoGainAttrValueNum, int *pMaxInfoGainAttrValue)
{
    ...
    //在计算各个属性的信息增益时用于存储属性变量的熵double entropyA=0,entropyV=0;

    //计算每个属性类型下共有多少种类型的属性变量int attributeVlaueTypeNum;
    //存储每个属性类型下的属性变量的值
    ...
    //初始化训练样例的第一个属性为信息增益最大的属性,其信息增益为 0 double maxInfoGain=0;
    ...
    *pMaxInfoGainAttrValue = *pNodeExamples->pTrainSample;
    ...
    //计算每个属性的信息增益,同时取出信息增益最大的属性及偏移for(i=0;i<pNodeExamples->attributeTypeNum;i++)
    {
        //属性的信息增益double infoGain;
        //取出第一个训练样本的属性变量的属性值存储
        *attributeValueName = *(pNodeExamples->pTrainSample + i);
        //属性变量个数初始化为 1 attributeVlaueTypeNum=1;
        //第一个属性变量的个数初始化为 1
        *attributeValueNum = 1;
        //判断第一个训练样本的目标属性的正负
        ...
        *attributeTargetValueNegNum = 1;
        ...
        for (j = 1; j < pNodeExamples->sampleNum; j++)
        {
            ...
            //把取出的属性值和之前存储的属性值做比较,看是否为新的属性值for(k=0;k<attributeVlaueTypeNum;k++)
            {
                //如果对比成功
                if (attributeValue == *(attributeValueName + k))
                {
                    ...
                    *(attributeValueNum + k) = *(attributeValueNum + k) + 1;
                    ...
                    if (*(pNodeExamples->pTrainSample + pNodeExamples->attributeTypeNum + j * (pNodeExample s->attributeTypeNum + 1)) == yes)
                    {
                        //正样本数加 1
            ...
            //如果没有对比成功,说明新出现了属性值if(compareFlag==0)
            {
                ...
                *(attributeTargetValueNegNum + attributeVlaueTypeNum) = 1;
                ...
                // 属 性 变 量 个 数 加 1 attributeVlaueTypeNum++;
            } //end if(compareFlag==0)
        }     //end for(j=1;j<*(pNodeExamples->pTrainSample);j++)

        //计算属性分类训练样例后熵的期望值entropyA=0;// 清 0 for(j=0;j<attributeVlaueTypeNum;j++)
        {
            //计算第 j 个属性变量的熵
            entropyV = CalcEntropy(*(attributeTargetValuePosNum + j), *(attributeTargetValueNegNum + j));
            entropyA = entropyA + (double)(*(attributeValueNum + j)) / (double)(pNodeExamples->sampleN um) * entropyV;
        }
        //计算属性的信息增益infoGain=entropyS-entropyA;
        //寻找信息增益最大的属性if(maxInfoGain<infoGain)
        {
            //更新最大信息增益的信息maxInfoGain=infoGain; maxInfoGainOffset=i;
...
{
    int i, j, k;
    //定义一个指针用于临时分配内存int *pPointTemp;
    //存储信息增益最高的属性的所有属性值int *pMaxInfoGainAttrValue;
    //存储信息增益最高的属性在训练样例中的列数
    int maxInfoGainOffset;
    //存储信息增益最高的属性包含的属性值个数(分支数) int maxInfoGainAttrValueNum;
    //定义一个决策树结点pNode pTreeNode;
    //存储训练样例目标属性为 yes 和 no 的样本数
    int posNum, negNum;

    //如果训练样例的样本数小于 1，说明输入参数有误，直接返回 NULL if(pNodeExamples->sampleNum<1)
    {
        return NULL;
    }
    //计算训练样例集的正样本数和负样本数
    CalcPosAndNegNum(pNodeExamples, &posNum, &negNum);
    //如果正样本数和负样本数都为 0，则出错if(posNum==0 && negNum==0)
    {
        return NULL;
    }
    //为决策树节点分配内存空间pTreeNode=malloc(sizeof(_node));
    //如果分配失败 ，返回错误
    if (pTreeNode == NULL)
    {
    ...
    //标识该节点为非叶子节点pTreeNode->lable=0xff;
    ...
    //如果属性数目小于 1，标识该节点叶子节点，且目标属性值为 yes 或 no 中比较多的一
    个
    //如果两者数目一样多，则目标属性值为 no if(pNodeExamples->attributeTypeNum<1)
    {
        if (posNum > negNum)
        {
        }
    ...
    //先给存储最大增益属性的属性值的指针分配和训练样例样本个数一样大小的临时空
    间
    ...
    //找出信息增益最高的属性
    maxInfoGainOffset = FindMaxInfoGainAttrbute(pNodeExamples, posNum, negNum, &maxInfo GainAttrValueNum, pPointTemp);
    //如果计算信息增益出错,则返回错误
    if (maxInfoGainOffset == -1)

    {
        //释放节点占用的内存空间free(pPointTemp); free(pTreeNode);
        return NULL;
    }
...
free(pPointTemp);
//把信息增益最高的属性名称存储到节点中
pTreeNode->attributeTypeName = *(
    pNodeExamples->pAttributeTypeName + maxInfoGainOff

                                            //存储节点的分支数即信息增益最高的属性的属性值种类数pTreeNode->childNum=maxInfoGainAttrValueNum;
                                            //存储每个子节点对应的属性值
                                            pTreeNode->pChildAttributeValue = pMaxInfoGainAttrValue;
    //先分配 maxInfoGainAttrValueNum 个指针赋给节点的子节点
    pTreeNode->pChildNode = (struct _node **)malloc(sizeof(struct _node

                                                               **) *
                                                    maxInfoGainAttrValueNum);
    if (pTreeNode->pChildNode == NULL) {
        free(pTreeNode);
        return NULL;
    }
    //清除子节点指向的指针变量for(i=0;i<maxInfoGainAttrValueNum;i++)
    {
...
//分配与训练样例集一样大小的临时空间来存储按每个属性值提取出的样本数据pPointTemp=malloc(pNodeExamples->sampleNum*(pNodeExamples->attributeTypeNum+1)

//按每个属性值依次提取样本数据for(i=0;i<maxInfoGainAttrValueNum;i++)
{
    ...
    } //end if(attrValueNum==0) else	//if(attrValueNum==0)
    {
        //从训练样例集中移出第 i 个属性值,重新组合成新的训练样例集
        int l;
        //定义一个节点的训练样例结构体 examples pExamples pNodeExamplesTemp;
        //为新的训练样例集分配内存空间
        ...
        //为训练样例结构体 examples 分配内存空间pNodeExamplesTemp=(pExamples)malloc(sizeof(_examples)); if(pNodeExamplesTemp==NULL)
        {
            free(pPointTemp);
            free(pTreeNode);
            return NULL;
...
pNodeExamplesTemp->pAttributeTypeName=malloc((pNodeExamples->attributeTypeNum-1
...
*(pNodeExamplesTemp->pAttributeTypeName+k)=*(pNodeExamples->pAttributeTypeName
...
//以 pNodeExamplesTemp 为训练样例集,以 pTreeNode 为父节点递归调用 ID3
 
算法
*(pTreeNode->pChildNode+i)=ID3(pNodeExamplesTemp); if(*(pTreeNode->pChildNode+i)==NULL)
{
                free(pPointTemp);
                free(pTreeNode);
                return NULL;
...
free(pNodeExamplesTemp->pAttributeTypeName); free(pNodeExamplesTemp->pTrainSample); free(pNodeExamplesTemp);
    } //end else	//if(attrValueNum==0)
} //end for(i=0;i<maxInfoGainAttrValueNum;i++)
//释放存储按每个属性值提取出的样本数据的临时空间free(pPointTemp);
//返回生成的节点return pTreeNode;
}
...
{
    int i;
    //定义一个新的字符串数组来存储到该节点之前的打印信息char prePrintString[MAX_ID3_CHAR_LENGTH];
    //先判断该节点是否为叶子节点if(pRootNode->lable!=0xff)
    {
        //如果是叶子节点,则把叶子节点的属性值(目标属性)添加到 printString 中并打印出
        来
            //strcat(printString,connectString); strcat(printString,attributeTargeteString[pRootNode->lable]); printf("%s\n",printString);
            return;
    }
    //把非叶子节点的属性类型添加到 printString 中
    strcat(printString, attributeTypeString[pRootNode->attributeTypeName]);
    //添加属性类型到属性值的连接符strcat(printString,connectString);
...
{
    int i;
    //如果是叶子节点,则释放该节点占用的内存空间if(pRootNode->lable!=0xff)
    {
        free(pRootNode);
        return;
    }
    //如果不是叶子节点for(i=0;i<pRootNode->childNum;i++)
    {
        //先递归调用 freeNode 释放每个子节点占用的内存空间
        freeNode(*(pRootNode->pChildNode + i));
    }
    //释放指向该非叶子节点的每个子节点的属性值占用的内存空间; free(pRootNode->pChildAttributeValue);
    //释放本节点占用的内存空间free(pRootNode);
    return;
}
//决策树算法程序void DecisionTree(void)
{
    //定义决策树的根节点pNode pRootNode=NULL;
    //定义根节点的训练样例集的属性类型
    _attributeTypeName pRootAttributeName[4] = {outlook, temperature, humidity, wind};
    //用原始训练实例初始化根节点的训练样例结构体 examples pExamples pRootExampls=(pExamples)malloc(sizeof(_examples)); if(pRootExampls==NULL)
    {
    ...
    //存储根节点的训练样例包含的属性类型数pRootExampls->attributeTypeNum=ATTR_NUM;
    //储根节点的训练样例
    ...
    //内存释放free(pRootExampls); if(pRootNode)
    {
        freeNode(pRootNode);
    }
    return;
